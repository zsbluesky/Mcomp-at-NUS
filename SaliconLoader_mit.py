...
def make_trainset(root):
    ...
    map_root = root / "train/maps"

    files = [f.stem for f in img_root.glob("*.jpeg")]

    images = []

    for f in files:
        img_path = (img_root / f).with_suffix(".jpeg")
        map_path = (map_root / f).with_suffix(".jpg")
        images.append([img_path, map_path])
...
def map_loader(path):
    name = str(path.stem).split('.')[0]
    name = name+'_fixMap.jpg'
    path = os.path.join('../mitdata2/train/maps', name)
...
class ImageList(data.Dataset):
    def __init__(self, root, transform=None, train=False,
                 loader=default_loader, map_loader=map_loader):
    ...
    def __getitem__(self, index):
        ...
        img_path, map_path = self.imgs[index]
